    ...
    def _load_model_cuda_faster(self) -> bool:
        ...
        try:
            ...
            if device.startswith("cuda"):
                # Voice extraction leaves temporary allocator blocks behind.
                # Release unused blocks once at startup, not between requests;
                # live voice prompts and CUDA graph pools stay allocated.
                try:
                    torch.cuda.empty_cache()
                except Exception:
                    logger.exception("Could not release unused warmup CUDA memory")
            self.load_error = None
            logger.info(
